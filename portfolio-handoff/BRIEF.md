...
- **Two accent colors, each scoped to a specific job:**
  - Gold/amber `#E2A13A` (hover `#F0B24C`, ink-on-accent `#1A1206`) — **buttons and tags, plus bold/`.hl` emphasis inside body copy**: CTA buttons, `.hl` inline highlights (e.g. "ADDIE", "Bloom's Taxonomy"), `<b>`/`<strong>` emphasis inside descriptive paragraphs (e.g. "Curriculum Design Document (CDD)"), `.objective-box`, `.flow-step.highlight`, `.accent-name` (the "Cheng" half of the hero name), and the image caption pills.
  - Neon blue `#3EB8FF` (`--heading` custom property, plus a `--heading-glow` text-shadow token for a subtle glow on large headings) — **headings and titles**: `h1`/`h2`/`h3`, `.eyebrow` labels, and `.stage-num`. Chosen after checking contrast against the `#0A0A0A` background (8.96:1, AAA even at small sizes) so it reads clearly rather than straining the eyes. `.accent-name` briefly shared this blue too (2026-09-14) but Francis asked for the orange back on the "bold" part of the title the same day, so it reverted to gold (no glow, to keep it crisp against the blue "Francis").
- **Body/descriptive copy:** site-wide default is `--ink-soft` (muted grey-blue), but on the **Portfolio page** (`portfolio.html`) the descriptive paragraphs read too dull against the near-black background, so `.case-intro .bio`, `.stage-body p`, and `.analyse-list` are bumped to full `--ink` (white), with any `<b>`/`<strong>` inside them set to the gold accent. This is scoped to those selectors specifically — it does **not** touch labels (`.eyebrow`, `.stage-num`), the profile page's hero bio, or anything in the nav/footer. If Francis asks for the same brightness bump elsewhere, extend this pattern rather than changing the global `p` rule.
...
- **Name treatment:** two-tone again, but not the original pairing — "Francis" renders in the heading neon blue (with the glow), "Cheng" (`.accent-name`) renders in the gold accent, no glow. Francis's own term for the accented half of the name is "bold" — when he says "make the title bold/orange," he means `.accent-name`, not literal `font-weight`.
