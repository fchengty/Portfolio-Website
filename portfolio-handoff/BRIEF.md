...
- **Two accent colors now, each scoped to a specific job:**
  - Gold/amber `#E2A13A` (hover `#F0B24C`, ink-on-accent `#1A1206`) — **buttons and tags only**: CTA buttons, the `.hl` inline body highlights, `.objective-box`, `.flow-step.highlight`, and the image caption pills. This used to also cover headings/titles — it no longer does (see below).
  - Neon blue `#3EB8FF` (`--heading` custom property, plus a `--heading-glow` text-shadow token for a subtle glow on large headings) — **headings and titles only**: `h1`/`h2`/`h3`, `.eyebrow` labels, `.stage-num`, and `.accent-name`. Chosen after checking contrast against the `#0A0A0A` background (8.96:1, AAA even at small sizes) so it reads clearly rather than straining the eyes.
...
- **Name treatment:** the hero name is no longer two-tone — "Francis Cheng" renders entirely in the heading neon blue (both the plain text and the `.accent-name` span share `--heading` now). The earlier two-tone version (white first name / gold surname) was superseded when titles moved to blue-only; if Francis wants a two-tone treatment back, it needs an explicit second color decision from him, not a reversion to gold.
